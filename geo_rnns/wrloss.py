...
class WeightMSELoss(Module):
    def __init__(self, batch_size, sampling_num):
        ...
        self.weightn = np.array(self.weight)
        sum = np.sum(self.weightn)
        self.weight = self.weight / sum
        # print(self.weight.shape)
        # print(np.stack(self.weight).shape)
        # print(type(self.weight))
        self.weight = Parameter(torch.from_numpy(self.weight).cuda(), requires_grad=False)
    ...
    def forward(self, input, target, isReLU=False):
        ...
        square_view = square.view(-1, 1)
        weight_view = self.weight.view(-1, 1)

        square_view = square_view.type(torch.cuda.DoubleTensor)
        weight_view = weight_view.type(torch.cuda.DoubleTensor)
        weight_square = torch.mul(square_view, weight_view)
